...
Firstly, instead of applying a global, uniform privacy budget to all tokens, we allocate noise proportionally to token-level re-identification risk. In DPMLM, a single $\epsilon$ is applied across the sentence, so all tokens are perturbed with the same budget even though their disclosure risk differs. Risk scores are estimated using explainability techniques over a text re-identification (TRI) attacker model, by default via SHAP-based attribution. Tokens that contribute more strongly to re-identification are assigned a lower effective $\epsilon$, making them more likely to be rewritten. Concretely, we use the heuristic $\epsilon_i \propto 1 / r_i$ with $\mathbb{E}_i[\epsilon_i] = \epsilon$, yielding
\begin{equation}
\epsilon_i = \epsilon \frac{1 / r_i}{\sum_{j=1}^{n} (1 / r_j)} = \frac{\epsilon}{\sum_{j=1}^{n} r_i / r_j}.
\end{equation}
The token-specific noise parameters are scaled such that their mean equals the global privacy budget, preserving the overall DP guarantee. PETRE is typically applied after explicit de-identification to handle indirect identifiers. The DPMLM-equivalent treatment is to assign detected direct identifiers near-zero $\epsilon$ (or equivalently very large risk), while we instead treat de-identification as a prerequisite and position our anonymization as a complementary step for the remaining tokens.
...
\hline
TAB & Original & Original & 0.8889 \\
TAB & De-identified using Presidio & De-identified using Presidio & 0.7857 \\
Reddit & Original & Original & 0.9750 \\
Reddit & De-identified using Presidio & De-identified using Presidio & 0.9500 \\
\hline
\end{tabularx}
