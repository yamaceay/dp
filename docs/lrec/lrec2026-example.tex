...
We use two datasets: TAB and PersonalReddit. TAB \cite{pilan_text_2022} provides a controlled setting for text re-identification with approximately 1.5k record samples that include text, metadata, and entity annotations, which enables PII detection training and evaluation. For privacy and utility assessment, we use the test split with 127 records from 126 distinct individuals. We define a lightweight downstream task for TAB, such as predicting macro-region or year of the court case, to quantify utility without adding new annotation work.
...
Standard machine learning practice requires disjoint training and evaluation sets to prevent information leakage. Training the TRI attacker directly on the target records would cause severe overfitting, yielding high accuracy without learning generalizable re-identification patterns. To address this, we construct proxy background knowledge for training: a separate corpus that represents prior writings or contextual information an adversary might realistically obtain before targeting specific individuals.

This proxy knowledge is ideally de-identified and lexically distinct from the target records, though it can be derived from original records when external data is unavailable. The key requirement is separation: TRI models are trained on background knowledge and evaluated on de-identified target records. This setup aligns with the static adversary assumption introduced in Section~\ref{sec:preliminaries}, where the attacker acquires contextual information prior to observing anonymized outputs. The lexical variation between training and evaluation data reduces memorization and ensures that measured re-identification risk reflects genuine linguistic patterns rather than surface-level overlap.
...
For TAB, the optimal setting used 10 training epochs with pretrained initialization, a batch size of 8, and a learning rate of $5 \times 10^{-5}$. 
...
\begin{tabularx}{\columnwidth}{|l|X|X|l|l|l|l|}
\hline
\textbf{Dataset} & \textbf{Background Knowledge (Train)} &  \textbf{Records (Eval)} & \textbf{MRR on Original Records} & \textbf{MRR on De-identified Records} & \textbf{Acc. on Original Records} & \textbf{Acc. on De-identified Records} \\
\hline
TAB & Original & Original & 0.965879 & 0.823675 & 0.937008 & 0.740157 \\
TAB & De-identified & De-identified & 0.945363 & 0.964173 & 0.905512 & 0.937008 \\
Reddit & Original & Original & 0.860740 & 0.681755 & 0.796190 & 0.573333 \\
Reddit & De-identified & De-identified & 0.874499 & 0.669367 & 0.826667 & 0.579048 \\
\hline
\end{tabularx}
\caption{TRI pipeline accuracy for original and de-identified settings on TAB and PersonalReddit.}
\end{table}

For DP-MLM, we use max\_retry\_rounds = 1 on PersonalReddit and max\_retry\_rounds = 5 on TAB. The TAB retry mechanism draws up to five samples from the same masked distribution and keeps the original token only if all samples match it.
...
Utility is measured with downstream supervision on TAB and PersonalReddit. We follow a setup--run--cleanup structure: in setup we instantiate the static TRI attacker and prepare the downstream splits, in run we evaluate anonymized records using task-specific heads, and in cleanup we release model artifacts. Downstream models reuse the TRI encoder by removing its prediction layer and attaching a task-specific head, then fine-tuning with a low learning rate ($5\times10^{-6}$ to $1\times10^{-5}$) to preserve the TRI representation while adapting to the task. We considered LoRA-style adapters, but we currently use standard fine-tuning given the moderate model sizes and the small number of update steps.

We map each target to a head based on the label type. Nominal labels use a softmax classifier. Ordinal labels use a CoRAL head \cite{cao_coral_2020} with shared weights and ordered thresholds. Cardinal labels use a single-node regression head. For TAB, we use \texttt{country\_group} (nominal) and \texttt{year\_group} (ordinal with fixed bins). Models are trained on the TAB dev split and validated on the TAB test split, then evaluated on anonymized outputs aligned by record index; we report the drop relative to the original test split (macro-F1 for country, macro-MAE for year). For PersonalReddit, we group samples by the attribute queried in the dialogue (e.g., age, education, occupation, income level, relationship status, and region), bin labels to balance frequencies, and train per-attribute heads. Because some attributes are small, we train on synthetic data and evaluate on original records to establish a stable baseline under the same head design.

\subsection{Text Similarity}

As a complementary signal, we compute the distance between original and anonymized texts to characterize the privacy-utility trade-off. We report cosine similarity as a lightweight baseline, and include BERTScore to capture contextual similarity.
...
\hline
\textbf{Method} & \textbf{Privacy (Rank)} & \textbf{Utility (Drop)} & \textbf{BERTScore} & \textbf{Runtime} \\
\hline
 &  &  &  &  \\
\hline
\end{tabularx}
\caption{Placeholder results table to be filled with privacy, utility, text similarity, and runtime metrics.}
...
% \begin{itemize}
%     \item TAB dataset: Epochs = 10, Pretraining, Batch size = 8, Learning rate = $5 \times 10^{-5}$. Reached $78.57\%$ accuracy.
