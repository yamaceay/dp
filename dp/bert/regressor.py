...
class BertRegressorHead(SupervisedDownstreamHead, BertHFPlumbing):
    def __init__(
        ...
        SupervisedDownstreamHead.__init__(
            self, name="bert_regressor", primary_metric=primary_metric
        )
    ...
    def fit(
        ...
        if self.normalize_targets:
            ...
            train_labels_normalized = (
                train_labels - self._target_mean
            ) / self._target_std
        ...
        print(
            f"Training on {len(train_labels)} samples: mean={train_labels.mean():.2f}, std={train_labels.std():.2f}"
        )
        print(
            f"Validating on {len(val_labels)} samples: mean={val_labels.mean():.2f}, std={val_labels.std():.2f}"
        )
        ...
        def _train_impl() -> None:
            ...
            train_dataset = EncodedDataset(
                train_encodings, train_labels_normalized, label_dtype=torch.float
            )
            val_dataset = EncodedDataset(
                val_encodings, val_labels_normalized, label_dtype=torch.float
            )
            ...
            def compute_metrics(eval_pred: EvalPrediction):
                ...
                r2 = (
                    float(1 - (np.sum((labels - preds) ** 2) / denom))
                    if denom > 0
                    else 0.0
                )
    ...
    def evaluate(self, x: Any, y: Sequence[Any]) -> Dict[str, float]:
        ...
        r2 = (
            float(1 - (np.sum((y_arr - predictions) ** 2) / denom))
            if denom > 0
            else 0.0
        )
        return {
