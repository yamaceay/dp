...
def build_shared_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--mode",
        type=str,
        choices=["utility", "privacy", "divergence"],
        help="Experiment mode override (optional)",
    )
    parser.add_argument(
        "--identifier",
        type=str,
        help="Optional model identifier override for utility experiments",
    )


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run experiments", argument_default=argparse.SUPPRESS
    )
    ...
    for name in ("utility", "divergence", "privacy"):
        subparser = subparsers.add_parser(
            name, help=f"Run {name} experiment", argument_default=argparse.SUPPRESS
        )
