...
def is_function(
    source: Dict[Any, Dict[Any, int]], target: Dict[Any, Dict[Any, int]]
) -> bool:
    functional_results_source = functional_results(source)
    functional_results_target = functional_results(target)
    return (
        functional_results_source["maps_to_one_target"]
        and functional_results_target["maps_to_multiple_targets"]
    )
        ...
        if total_items <= head_n + tail_n:
            ...
            for idx, (src_key, tgt_dict) in enumerate(items):
                if idx < head_n or idx >= total_items - tail_n:
                    ...
                    result += (
                        f"  ... ({total_items - head_n - tail_n} more items) ...\n"
                    )
        return result
...
class FunctionalAnalysis:
    def __init__(
        self, records: List[DatasetRecord], value_getters, exclude_keys: List[str] = []
    ) -> None:
        ...
        for i in range(len(self.data_keys)):
            for j in range(i + 1, len(self.data_keys)):
                ...
                if len(self.data[source]) != len(self.data[target]):
                    raise ValueError(
                        f"Source and target data length mismatch: {source} ({len(self.data[source])}) vs {target} ({len(self.data[target])})"
                    )
                        ...
                        for src_key, mid_dict in f1.by_source.items():
                            for mid_key, count in mid_dict.items():
                                if mid_key in f2.by_source:
                                    for tgt_key, tgt_count in f2.by_source[
                                        mid_key
                                    ].items():
                                        ...
                                        new_by_source[src_key][tgt_key] += (
                                            count * tgt_count
                                        )
                        new_functional = Functional(f1.source, f2.target)
                        new_functional.add(new_by_source)
                        if not functional_registry.is_functional_registered(
                            new_functional
                        ):
