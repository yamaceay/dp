...
class RiskAnonymizer(Anonymizer):
    ...
    def __init__(
        self, *args, risk_temperature: float = 1.0, mask_text: str = "[MASK]", **kwargs
    ):
    ...
    def pre_stream_anonymize(
        self,
        *args,
        texts_or_indices: Optional[Union[List[str], List[int]]] = None,
        ...
        **kwargs,
        ...
        if not isinstance(record_names, list) or len(record_names) != len(
            texts_or_indices
        ):
            raise ValueError(
                "record_names must be provided and aligned with texts_or_indices"
            )
    ...
    def anonymize_any_text(
        ...
        **kwargs,
    ) -> List[Tuple[Dict[str, Any], AnonymizationResult]]:
        if buckets is None:
            buckets = []
        text_hash = self.hash_text(text)
        ...
        if len(buckets) != 1 or not isinstance(buckets[0], RhoParams):
            raise ValueError(
                "RiskAnonymizer only supports RhoParams for grid anonymization."
            )
                ...
                (
                    ...
                    AnonymizationResult(
                        ...
                        annotations=TextAnnotations(
                            spans=result_spans, token_edits=token_edits
                        ),
                        metadata=metadata,
                    ),
                )
    ...
    def _compute_scores(
        self, text: str, spans: List[Tuple[int, int]], record_name: Optional[str]
    ) -> np.ndarray:
        precomputed = self._lookup_precomputed_scores(spans, record_name)
        if precomputed is not None:
            return precomputed
        raise NotImplementedError(
            "RiskAnonymizer requires precomputed risk scores for each record."
        )

    def _precomputed_scores_and_spans(
        self, record_name: Optional[str]
    ...
    def _lookup_precomputed_scores(
        self, spans: List[Tuple[int, int]], record_name: Optional[str]
    ) -> Optional[np.ndarray]:
        if record_name and record_name in self._risk_scores_by_uid:
            ...
            for start, end in spans:
                if (start, end) not in mapping:
                    print(
                        f"Missing score for span ({start}, {end}) in record {record_name}"
                    )
                    return None
