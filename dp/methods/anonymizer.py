...
class Anonymizer(ABC):
    ...
    def anonymize_from_dataset(
        self, idx: int, *args, buckets: Buckets = [], **kwargs
    ) -> List[Tuple[BucketDict, AnonymizationResult]]:
        raise NotImplementedError()

    def anonymize(
        self, text_or_idx: Union[str, int], *args, buckets: Buckets = [], **kwargs
    ) -> List[Tuple[BucketDict, AnonymizationResult]]:
        if isinstance(text_or_idx, str):
            return self.anonymize_any_text(
                text_or_idx, *args, buckets=buckets, **kwargs
            )
        return self.anonymize_from_dataset(
            text_or_idx, *args, buckets=buckets, **kwargs
        )
    ...
    def stream_anonymize(
        self,
        texts_or_indices: Union[List[str], List[int]],
        *args,
        buckets: Buckets = [],
        **kwargs,
    ) -> Iterator[List[Tuple[BucketDict, AnonymizationResult]]]:
        for text in tqdm(
            texts_or_indices,
            desc="Anonymizing texts",
            unit="text",
            total=len(texts_or_indices),
        ):
