...
class _RatBenchAttackerDatasetAdapterBase(AttackerDatasetAdapter):
    """TRI background-knowledge adapter for RAT-Bench.

    `test_texts` (what the trained TRI classifier is evaluated/attacked against)
    is always the real record's literal text, regardless of variant. Only
    `train_texts`/`eval_texts` (what the classifier is fine-tuned on) differ:
    the default (bart-enabled) subclass uses a BART summary of the record text,
    matching how `tab`/`db_bio` background knowledge is generated; the
    `nobart` subclass uses the literal record text instead, as the ablation.
    Records always come from the real `rat_bench` loader; there is no separate
    "rat_bench_bart" dataset.
    """
    ...
    def __init__(
        ...
        **data_kwargs,
        ...
        adapter = get_adapter("rat_bench", **data_kwargs)
    ...
    def iter_records(self, progress: bool = False) -> Iterable[AttackerDatasetRecord]:
        ...
        for idx, record in enumerate(iterator):
            ...
            if self._cache_map is not None and modified_record.name in self._cache_map:
                ...
                train_texts = [modified_record.text] * self.n_train_samples
                eval_texts = [modified_record.text] * self.n_eval_samples
                test_texts = []
            if not test_texts:
...
class RatBenchAttackerDatasetAdapter(_RatBenchAttackerDatasetAdapterBase):
    USE_BART = True


class RatBenchNoBartAttackerDatasetAdapter(_RatBenchAttackerDatasetAdapterBase):
    USE_BART = False


__all__ = ["RatBenchAttackerDatasetAdapter", "RatBenchNoBartAttackerDatasetAdapter"]
