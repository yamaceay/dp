...
class TokenAwareChunker:
    ...
    def _find_target_token_index(
        self, offsets: List[Tuple[int, int]], span: Tuple[int, int]
    ) -> Optional[int]:
        ...
        if self.overlap_tokens > 0:
            ...
            if candidates:
                ...
                return min(
                    candidates,
                    key=lambda c: (
                        abs((((int(c.start) + int(c.end)) / 2.0) - target_center)),
                        int(c.start),
                    ),
                )
        ...
        while i < len(tokens):
            ...
            if offsets:
                ...
                while end_idx > i:
                    ...
                    chunk_tokens = self.tokenizer(chunk_text, add_special_tokens=False)[
                        "input_ids"
                    ]
