...
class PIIDetector:
    def __init__(
        ...
        if self.labels:
            print(
                f"✓ PIIDetector initialized with {len(self.labels)} labels on {self.device}"
            )
        else:
    ...
    def _load_pretrained_model(self) -> None:
        ...
        if os.path.isdir(model_path):
            checkpoints = [
                d for d in os.listdir(model_path) if d.startswith("checkpoint-")
            ]
            if checkpoints:
    ...
    def _initialize_labels(self, labels: Optional[List[str]] = None) -> None:
        if labels is not None:
            ...
            if hasattr(config, "id2label") and config.id2label:
                ...
                self.label_to_id = {v: k for k, v in self.id_to_label.items()}
                self.labels = [
                    self.id_to_label[i] for i in sorted(self.id_to_label.keys())
                ]

        else:
            raise ValueError(
                "Labels must be provided or model must be loaded to initialize labels"
            )
        ...
        **kwargs,
        ...
        if eval_dataset is not None:
            if use_nervaluate and NERVALUATE_AVAILABLE:
                resolved_modes = (
                    nervaluate_modes
                    if nervaluate_modes is not None
                    else ["strict", "exact", "partial", "type"]
                )
    ...
    def predict(
        self, records: List[DatasetRecord], verbose: bool = False
    ) -> List[DatasetRecord]:
        if not records:
            return []
        ...
        for record in records:
            if self.force_non_o_prediction:
                ...
                if self.aggregation_strategy == "none":
                    ...
                    span_dicts = process_with_chunking(
                        record.text, self.chunker, detect, aggregator
                    )
                spans = [
                    TextAnnotation(
                        start=s["start"],
    ...
    def evaluate(
        ...
        unique_entity_types = sorted(
            list(set([label[2:] for label in self.labels if label.startswith("B-")]))
        )
        ...
        if use_nervaluate and NERVALUATE_AVAILABLE:
            ...
            for true_record, pred_record in zip(records, predicted_records):
                true_bio = self._spans_to_token_labels(
                    true_record.text, true_record.spans or []
                )
                pred_bio = self._spans_to_token_labels(
                    pred_record.text, pred_record.spans or []
                )
            ...
            if "per_category" in metrics and metrics["per_category"]:
                ...
                if "partial" in metrics["per_category"]:
                    for tag, tag_metrics in metrics["per_category"]["partial"].items():
                        print(
                            f"    {tag}: P={tag_metrics['precision']:.4f}, R={tag_metrics['recall']:.4f}, F1={tag_metrics['f1']:.4f}, Support={tag_metrics['support']}"
                        )
                else:
    ...
    def _compute_metrics_ner(self, eval_pred, modes: List[str], primary_mode: str):
        ...
        predictions, labels = eval_pred
        true_preds, true_labels = self._get_preds_and_labels_for_nervaluate(
            predictions, labels
        )
    ...
    def _spans_to_token_labels(
        ...
        for span in sorted(spans, key=lambda s: s.start):
            ...
            token_start = (
                char_to_token[span.start] if span.start < len(char_to_token) else None
            )
            token_end = (
                char_to_token[span.end - 1]
                if span.end > 0 and span.end - 1 < len(char_to_token)
                else None
            )
