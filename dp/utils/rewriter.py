...
class BartRewriter(Rewriter):
    def __init__(
        ...
        self.rewriting_pipeline = pipeline(
            "summarization", model=model_name, device=device
        )
    ...
    def rewrite(
        self, text: str, max_length: int = 150, min_length: int = 40, **kwargs
    ) -> str:
        ...
        for chunk in chunks:
            summary = self.rewriting_pipeline(
                chunk.text, max_length=max_length, min_length=min_length, **kwargs
            )
