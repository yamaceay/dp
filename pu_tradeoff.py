...
def metric_name_and_unit(metric_name: str) -> tuple[str, str | None]:
    ...
    if metric_name in {"U_acc", "PU_acc"}:
        return metric_name.replace("_acc", ""), "ACC"
    if metric_name in {"U_mae", "PU_mae"}:
...
def y_axis_assessment_label(y_column: str, metric_bounds: dict[str, tuple[float, float]]) -> str:
    ...
    if y_column.startswith("PU_"):
        return f"Pseudo-utility ({metric_label})"
...
def y_metric_context(y_column: str) -> tuple[str, str] | None:
    ...
    if y_column == "U_mae":
        return ("utility", "mae")
    if y_column == "PU_acc":
        return ("pseudo_utility", "acc")
    if y_column == "PU_mae":
        return ("pseudo_utility", "mae")
    return None
...
def raw_metric_column(metric_name: str) -> str | None:
    raw_columns = {
        ...
        "PU_acc": "pseudo_utility_utility_nominal_raw_acc",
        "PU_mae": "pseudo_utility_utility_ordinal_raw_mae",
...
def resolve_plot_targets(args: argparse.Namespace) -> list[tuple[str, str, str, str]]:
    ...
    y_options = [
        ("u_nominal", "u_nominal"),
        ("u_ordinal", "u_ordinal"),
        ("pu_nominal", "pu_nominal"),
        ("pu_ordinal", "pu_ordinal"),
        ("d_bertscore", "d_bertscore"),
        ("d_cosine", "d_cosine"),
        ("d_pp", "d_pp"),
        ("t_anon_avg_s", "t_anon_avg_s"),
...
def _run_meta_tradeoff(selected_datasets: list[str], pairs: list[tuple[str, str]]) -> None:
    from plot_mega_shared import (
        GROUP_ORDER, VARIANTS, VariantStats, compute_variant_stats,
        load_meta, read_dataset_labels,
    )
    import matplotlib
    matplotlib.use("Agg")
    from matplotlib import pyplot as plt
    ...
    if not raw:
        print("No meta_logs.csv found for meta tradeoff.")
        return

    for x_col, y_col in pairs:
        panel_data: dict[str, tuple[list[VariantStats], list[VariantStats]]] = {}
        for ds, df in raw.items():
            if x_col not in df.columns or y_col not in df.columns:
                continue
            xs = compute_variant_stats(df, x_col)
            ys = compute_variant_stats(df, y_col)
            if xs and ys:
                panel_data[ds] = (xs, ys)
        ...
        fig, axes = plt.subplots(1, n_datasets, figsize=(7.0 * n_datasets, 6.5),
                                 squeeze=False)
        fig.subplots_adjust(wspace=0.35)

        for ax, (ds, (x_stats, y_stats)) in zip(axes[0], panel_data.items()):
            x_by_label = {s.variant.label: s for s in x_stats}
            y_by_label = {s.variant.label: s for s in y_stats}
            common = [v for v in VARIANTS if v.label in x_by_label and v.label in y_by_label]

            for v in common:
                sx, sy = x_by_label[v.label], y_by_label[v.label]
                color = v.color
        ...
        legend_handles = [
            Line2D([0], [0], color="#888888", linewidth=4.0, alpha=0.5,
                   label="Range (min–max)"),
        ...
        out = meta_dir / f"{x_col.lower()}_{y_col.lower()}.pdf"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--all", action="store_true", help="Generate all x/y metric combinations")
    parser.add_argument("--meta", action="store_true", help="Meta tradeoff: P vs U/SS/D from meta_logs.csv")
    ...
    parser.add_argument("--experiment", nargs="*", dest="experiments", metavar="EXPERIMENT",
                        help="Method group names to include (default: all)")
    ...
    selected_datasets = args.datasets if args.datasets else dataset_names
    selected_experiments: set[str] | None = set(args.experiments) if args.experiments else None
    ...
    for dataset_name in selected_datasets:
        ...
        if selected_experiments is not None:
            method_names = [n for n in method_names if n in selected_experiments]
    ...
    for dataset_name, method_name, x_name, x_column, y_name, y_column in plan:
        ...
        df = df[~df["method"].isin(["baseline", "dummy"])].copy()
        if df.empty:
            skipped_count += 1
            progress.update(1)
            continue
        ...
        reference_points: dict[str, tuple[float, float] | None] = {}
        for _ref in REFERENCE_STYLES:
            if _ref == "presidio" and method_is_excluded("presidio", excluded_methods):
                reference_points[_ref] = None
                continue
            _pt = method_point_from_any_metric_schema(reference_source_df, _ref, x_column, y_column)
            if _pt is None:
                _pt = transformed_method_point(
                    dataset_name, _ref, x_column, y_column,
                    fallback_frames=[reference_source_df],
                )
            reference_points[_ref] = _pt

        for _pt in reference_points.values():
            if _pt is not None:
                extra_y_values = list(extra_y_values) + [_pt[1]]
        ...
        for ax, epsilon_value, panel_df in zip(axes, panel_values, panel_frames):
            ...
            for _ref, (_lbl, _mkr, _sz, _col) in REFERENCE_STYLES.items():
                _rpt = reference_points.get(_ref)
                if _rpt is None:
                    continue
                sc = ax.scatter(*_rpt, marker=_mkr, s=_sz, color=_col, zorder=7, label=_lbl)
                if _lbl not in seen_legend_labels:
                    seen_legend_labels.add(_lbl)
        ...
        if non_empty:
            ...
            extra_x = [pt[0] for pt in reference_points.values() if pt is not None] or None
            extra_y = list(extra_y_values)
            x_limits, y_limits = zoomed_limits_for_df(combined_df, extra_y_values=extra_y or None, extra_x_values=extra_x)
            for ax in axes:
