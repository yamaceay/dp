...
def metric_category(metric_name: str) -> str:
    for prefix in ("privacy_", "utility_", "pseudo_utility_", "divergence_", "runtime_"):
...
def main() -> None:
    ...
    parser.add_argument("--metrics", nargs="*", default=None, metavar="METRIC",
                        help="Metric names to include (default: all)")
    parser.add_argument("--experiments", nargs="*", default=None, metavar="EXPERIMENT",
                        help="Method set names to include (default: all)")
    ...
    selected_metrics: set[str] | None = set(args.metrics) if args.metrics else None
    ...
    for dataset_name in selected_datasets:
        ...
        if not args.meta:
            for method_set in active_method_sets:
        ...
        for metric_spec in dataset_spec["scores"]:
            ...
            if selected_metrics is not None and metric_name not in selected_metrics:
                progress.update(2 * len(active_method_sets))
                continue
