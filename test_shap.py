...
if __name__ == "__main__":
    ...
    explainer_type = ShapType(args.explainer)
    explainer = ShapExplainer(model_name=args.explainer_in, explainer_type=explainer_type)
    ...
    for idx, (record, text) in enumerate(tqdm(zip(records, result_texts), desc="Explaining records", total=len(records))):
        ...
        explain_kwargs = dict(target_label=target_label)
        scores = explainer.explain(text, offsets, **explain_kwargs)
        clear_memory()
