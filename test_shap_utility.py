...
class UtilityShapConfig:
    ...
    def set_max_samples(self, max_samples: int) -> UtilityShapConfig:
...
class DatasetSampler:
    ...
    def sample(self, max_samples: int) -> List[Tuple[str, str, Dict]]:
        ...
        for record in self.adapter.iter_records():
            ...
            if not text or feature_label is None:
                continue
            samples.append((text, str(feature_label), dict(record.metadata)))
        return samples
...
class ShapAnalyzer:
    ...
    def analyze(self) -> List[Dict]:
        print(f"[analyze] sampling up to {self.config.max_samples} records for feature={self.config.feature}")
        samples = self.sampler.sample(self.config.max_samples)
        if not samples:
        ...
        for idx, (text, label, metadata) in enumerate(samples):
            ...
            results.append({
                ...
                "top_5_tokens": token_importance[:5],
            })
            
            print(f"  label={label}, top tokens:")
            for item in token_importance[:5]:
...
def main() -> None:
    ...
    config.set_max_samples(5)
    config.set_explainer_type(ShapType.DEFAULT)
    print(f"[main] config: feature={config.feature} max_samples={config.max_samples}")
    config.validate()
