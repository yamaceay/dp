...
param_pattern = r"configs/runtime/(.*?)/(.*?)\.yaml"
...
for dataset, pdir, file in files:
    with open(file, "r") as f:
        ...
        with open("slurm/states/" + pdir + "/" + command + ".state", "r") as sf:
            ...
            runtimes_grouped = {}
            for runtime in runtime_in:
                params_match = re.search(param_pattern, runtime)
                if not params_match:
                    raise ValueError(f"Runtime param pattern not matched in {file}")
                
                name = params_match.group(2)
                param, value = name.split("_", 1)
                param = "epsilon" if param == "eps" else param
                if param == "epsilon":
                    value = int(value)
                    runtimes_grouped |= {param: value}
                    continue
                
                if "*" in value:
                    all_paths = []
                    for f in os.listdir(Path(runtime).parent):
                        runtime_base = os.path.basename(runtime)
